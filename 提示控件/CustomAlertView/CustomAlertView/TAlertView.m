...
@property (strong, nonatomic) void (^callBackBlock)(TAlertView *alertView, NSInteger buttonIndex,NSString *fieldtext);
...
@property (strong, nonatomic) UITextField               *messageLabel;
...
-(id)init
{
    return [self initWithTitle:nil message:nil buttons:nil andCallBack:^(TAlertView *alertView, NSInteger buttonIndex,NSString *fieldtext){}];
}

-(id)initWithTitle:(NSString*)title andMessage:(NSString*)message
{
    return [self initWithTitle:title message:message buttons:nil andCallBack:^(TAlertView *alertView, NSInteger buttonIndex,NSString *fieldtext){}];
}

-(id)initWithTitle:(NSString*)title message:(NSString*)message buttons:(NSArray*)buttons andCallBack:(void (^)(TAlertView *alertView, NSInteger buttonIndex,NSString *fieldtext))callBackBlock;
{
    if (self = [super init]) {
...
-(void)addSubviewsWithOverlay:(BOOL)withOverlay
{
    ...
    _messageLabel = [[UITextField alloc] initWithFrame:CGRectMake(insideHorisontalMargin, yPos, aletViewFrame.size.width - 2*insideHorisontalMargin, 0)];
    _messageLabel.placeholder = _message;
    _messageLabel.borderStyle = UITextBorderStyleRoundedRect;
    _messageLabel.font = _messageFont;
    _messageLabel.textColor = _messageColor;
    _messageLabel.textAlignment = NSTextAlignmentLeft;
//    _messageLabel.text = _message;
    CGRect lab = _messageLabel.frame;
    lab.size.height = 30;
    _messageLabel.frame = lab;
...
-(void)buttonTapped:(UITapGestureRecognizer*)tapGestureRecognizer
{
    ...
    if (_callBackBlock) {
        _callBackBlock(self, buttonIndex,_messageLabel.text);
    }
